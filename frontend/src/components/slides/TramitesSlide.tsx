...
import { Search, Chevron, Card } from '../icons';
...
function AccordionItem({ tr }: { tr: Tramite }) {
  ...
  return (
    <div className={`mb-2.5 overflow-hidden rounded-2xl border bg-paper transition ${open ? 'border-peru shadow-soft' : 'border-line hover:border-line-2'}`}>
      ...
      >
        <span className="min-w-[52px] flex-none rounded-md bg-peru px-2 py-1.5 text-center font-mono text-[13px] font-bold tracking-[.3px] text-white">
          {tr.codigo}
        </span>
        <span className="min-w-0 flex-1">
          <h3 className="text-[16px] font-bold leading-tight tracking-[-.2px]">{tr.titulo}</h3>
          <span className="mt-0.5 block text-xs font-semibold text-ink-3">{tr.categoria}</span>
        </span>
...
export function TramitesSlide() {
  ...
  const filtered = useMemo(() => {
    ...
    return tramites.filter((t) => {
      ...
      if (!q) return true;
      const hay = `${t.titulo} ${t.categoria} ${t.codigo} ${t.requisitos.map((r) => r.texto + (r.sub?.join(' ') ?? '')).join(' ')}`.toLowerCase();
      return hay.includes(q);
    });
  }, [tramites, query, category]);

  return (
    <SlideShell eyebrow="SECCIÓN 02" title="Tipos de trámite y requisitos"
      lead="Toca cada trámite para desplegar sus requisitos. Filtra por categoría o escribe para encontrarlo más rápido.">
      <div className="sticky top-0 z-[5] my-4 flex flex-wrap items-center gap-2.5 bg-gradient-to-b from-surface to-transparent py-2.5">
        <label className="flex h-11 flex-1 min-w-[220px] items-center gap-2.5 rounded-xl border border-line bg-paper px-3 focus-within:border-peru focus-within:ring-2 focus-within:ring-peru-tint">
          ...
          <input
            ...
            placeholder="Buscar trámite… (ej. menores, renovación, matrimonio)"
      ...
      <div className="mt-[18px] pt-1">
        {filtered.length === 0 ? (
          ...
          filtered.map((tr) => <AccordionItem key={tr.codigo + tr.titulo} tr={tr} />)
        )}
      </div>

      <div className="mt-4 flex items-start gap-3 rounded-xl border note-amber bg-amber-tint px-4 py-3.5">
        <Card width={20} height={20} className="mt-0.5 flex-none text-amber" />
        <p className="text-sm leading-relaxed text-ink">
          <b>Fotografía (aplica a casi todos los trámites):</b> tamaño pasaporte, no porosa, fondo blanco, sin sonrisa ni gestos, boca cerrada, sin prendas en la cabeza (salvo por motivos religiosos) y sin anteojos o lentes.
        </p>
      </div>
    </SlideShell>
  );
